...
export default function TransactionHistory() {
  ...
  useEffect(() => {
    // ดึงข้อมูลประวัติจริงจากระบบแยกตามบัญชีเบอร์โทรศัพท์
  ...
  return (
    // กำหนด h-screen และ flex-col เพื่อให้พื้นที่เนื้อหาเลื่อนได้ภายในหน้าจอ
    <div className="flex flex-col h-screen bg-[#F8FAFB] font-sans overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 scrollbar-hide">
        ...
        <div className="bg-white rounded-[1.5rem] shadow-sm border border-gray-100 overflow-hidden mb-6">
          ...
          {/* Header แถวบนสุด: ล็อคไว้ด้านบนเมื่อมีการเลื่อน (Sticky Header) */}
          ...
          <div className="divide-y divide-gray-50">
            {transactions.length > 0 ? (
              transactions.map((item) => (
                <div key={item.id} className="grid grid-cols-12 px-6 py-5 items-center hover:bg-gray-50/50 active:bg-gray-100 transition-colors">
