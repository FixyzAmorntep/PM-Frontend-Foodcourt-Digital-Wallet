...
export default function VATModal({ isOpen, onClose, value, onSave }: any) {
  // 🚩 1. สร้าง State พักข้อมูลไว้ใน Modal เอง
  const [tempValue, setTempValue] = useState(value);
  const [mounted, setMounted] = useState(false);

  useEffect(() => {
    setMounted(true);
    // 🚩 2. เมื่อเปิด Modal ให้ดึงค่าจริงจากข้างนอกมาพักไว้ที่นี่
    if (isOpen) setTempValue(value);
    return () => setMounted(false);
  }, [isOpen, value]);

  // 🚩 3. ฟังก์ชันเช็คค่าและเก็บไว้ในที่พัก (ยังไม่ส่งออกไปข้างนอก)
    ...
    <div style={{
      ...
      <div className="relative bg-white rounded-[2.5rem] w-2/3 shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-200">
        ...
        <div className="px-8 pt-8 flex justify-between items-center text-slate-800">
          <h3 className="text-[18px] font-black uppercase tracking-tight italic">Edit %VAT</h3>
        ...
        <div className="px-8 py-6">
          <p className="text-[11px] text-[#4F7A73] font-bold uppercase mb-4">
          ...
          <div className="bg-[#F8FAFB] rounded-[1.5rem] py-10 flex items-center justify-center gap-2 border border-slate-100">
            <input 
              type="number" 
              value={tempValue} // 🚩 ใช้ค่าที่พักไว้ในนี้
        ...
        <div className="px-8 pb-12 flex flex-col gap-4">
          <button 
            // 🚩 4. เมื่อกด Save ค่อยส่งค่าที่พักไว้กลับไปอัปเดตข้างนอก
          ...
          <button 
            onClick={onClose} // 🚩 กด Cancel ก็แค่ปิดหน้าต่าง ค่าจริงข้างหลังไม่เปลี่ยน
            className="w-full text-center font-bold text-slate-400 hover:text-slate-600 transition-all text-[11px] uppercase tracking-widest"
          >
            Cancel
          </button>
        </div>
